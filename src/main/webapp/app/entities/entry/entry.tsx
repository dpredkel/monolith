...
export const Entry = (props: IEntryProps) => {
  ...
  return (
    <div>
      <h2 id="entry-heading">
        <Translate contentKey="monolithApp.entry.home.title">Entries</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="monolithApp.entry.home.createLabel">Create new Entry</Translate>
        </Link>
      </h2>
          ...
          {entryList && entryList.length > 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('title')}>
                    <Translate contentKey="monolithApp.entry.title">Title</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('content')}>
                    <Translate contentKey="monolithApp.entry.content">Content</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('date')}>
                    <Translate contentKey="monolithApp.entry.date">Date</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="monolithApp.entry.blog">Blog</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {entryList.map((entry, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{entry.blogName ? <Link to={`blog/${entry.blogId}`}>{entry.blogName}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="monolithApp.entry.home.notFound">No Entries found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
